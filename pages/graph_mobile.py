...
_y_range = st.slider(
    ...
    value=(float(st.session_state.y_min_m), float(st.session_state.y_max_m)),
    step=0.1, format="%.1f억",
)
st.session_state.y_min_m = _y_range[0]
st.session_state.y_max_m = _y_range[1]

# ── 기간 선택 (radio) ──────────────────────────
_period_keys = list(X_OPTS.keys())
_cur_idx = _period_keys.index(st.session_state.x_range_m)
_selected = st.radio(
    "기간", _period_keys,
    index=_cur_idx,
    horizontal=True,
