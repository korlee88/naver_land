...
REGION_KEYWORDS: dict[str, list[str]] = {
    "41220": [],  # 평택시 — 전체 수집 (단지 미지정). 예전엔 관심 단지 키워드로만 걸러 받았으나,
                  # 국토부 실거래가의 공식 등록 단지명은 마케팅명과 표기/순서가 다를 수 있어
                  # "평택센트럴자이 2단지"(내 매수 단지) 포함 상당수가 키워드 불일치로 조용히
                  # 누락됐을 가능성이 높다 — 안성시와 같은 이유로 전체 수집으로 통일.
    "43113": [],  # 청주 흥덕구 — 전체 수집 (단지 미지정)
    "43114": [],  # 청주 청원구 — 전체 수집 (단지 미지정)
    "41550": [],  # 안성시 — 전체 수집 (단지 미지정). 예전엔 "공도"/"우미린"만 걸러 받아
                  # 그 두 키워드가 안 들어간 안성시 단지는 실거래가 있어도 저장 자체가
                  # 안 됐다 — 다른 지역(청주/오산/화성/수원)과 같은 방식으로 통일.
    "41370": [],  # 오산시 — 전체 수집 (단지 미지정)
    # 화성시는 2026-02-01부로 만세구·효행구·병점구·동탄구 4개 구로 분구되면서
    # 기존 통합 코드(41590)는 실거래 신고 조회에 더 이상 잡히지 않음 — 4개 구 코드로 대체
    "41591": [],  # 화성시 만세구 — 전체 수집 (단지 미지정)
    "41593": [],  # 화성시 효행구 — 전체 수집 (단지 미지정)
    "41595": [],  # 화성시 병점구 — 전체 수집 (단지 미지정)
    "41597": [],  # 화성시 동탄구 — 전체 수집 (단지 미지정)
    "41111": [],  # 수원시 장안구 — 전체 수집 (단지 미지정)
    "41113": [],  # 수원시 권선구 — 전체 수집 (단지 미지정)
    "41115": [],  # 수원시 팔달구 — 전체 수집 (단지 미지정)
    "41117": [],  # 수원시 영통구 — 전체 수집 (단지 미지정)
}
